...
function checkForDaily() {
	...
	showToast(
		'<div style="text-align: center">Already played daily Wordle <br> press to play a random Wordle?</div>',
		'5000',
		'info',
		() => playRandom()
	);
...
function colorRow(letterColorPair) {
	for (const key in letterColorPair) {
		...
		let sDelay =
			parseFloat($(':root').css('--animation-flip-duration')) *
				(key % WORD_LENGTH) *
				0.9 +
			's';
		let delay =
			(parseFloat(sDelay) +
				parseFloat($(':root').css('--animation-flip-duration')) / 2) *
			1000;
		let fullDuration =
			delay +
			parseFloat($(':root').css('--animation-flip-duration')) * 1000;
		...
		setTimeout(() => {
			$(letter)
				...
				.attr('state', letterColorPair[key]);
		...
		if (key % WORD_LENGTH == 4) return fullDuration;
	}
}
...
function colorKey(whichLetter, color) {
	...
	$(keyboard).each(function (index, element) {
		if ($(element).text().toUpperCase() != whichLetter) return;
		if (
			$(element).attr('taged') == 'grey' ||
			$(element).attr('taged') == 'green'
		)
			return;
		if ($(element).attr('taged') == 'yellow' && color == 'grey') return;

		$(element)
			.css({ backgroundColor: cssVar, color: 'var(--state-font)' })
			.attr('taged', color);
	});
}
...
function submitGuess() {
	...
	let guessedWord = grid[guessIndex].reduce(
	...
	TO_GUESS.split('').forEach((char) => {
		availableLetters[char] = availableLetters[char]
	...
	grid[guessIndex].forEach((letter, i) => {
		if (letter == TO_GUESS.charAt(i)) {
			...
			availableLetters[letter] = availableLetters[letter]
				? availableLetters[letter] - 1
				: 0;
		} else letterColorPair[guessIndex * WORD_LENGTH + i] = 'grey';
	});
	...
	setTimeout(() => {
		...
		if (guessedWord == TO_GUESS) {
			...
			saveGame('1');
			showToast(
				'<div style="text-align: center">You won GG<br>Press to play a random Wordle?</div>',
				'3000',
				'success',
				() => {
					playRandom();
		...
		if (guessIndex == NUMBER_OF_GUESSES) {
			...
			saveGame('0');
			showToast(
				`<div style="text-align: center">You lost: <p style="font-weight: 900">${TO_GUESS}</p> was the correct word<br>Press to play a random Wordle?</div>`,
				'5000',
				undefined,
				() => playRandom()
			);
...
function handleKeyboardInput(pressedKey) {
	...
	if (pressedKey.key == 'Control') fullClear = false;
	...
	if (pressedKey.key === 'Backspace' || pressedKey.key === 'Delete') {
		if (fullClear) {
			fullClear = false;
			letterIndex = 5;
			while (letterIndex != 0) removeLetter();
		} else removeLetter();
		return;
	}
...
function playRandom() {
	...
	TO_GUESS =
		TARGET_WORDS[
			Math.floor(Math.random() * TARGET_WORDS.length) %
				TARGET_WORDS.length
		].toUpperCase();
...
function saveGame(isWin) {
	...
	localStorage.setItem(
		...
		localStorage.getItem('played') != null
			? parseInt(localStorage.getItem('played')) + 1
			: 1
	);
	...
	if (isWin == '1') {
		...
		localStorage.setItem(
			guessed,
			localStorage.getItem(guessed) != null
				? parseInt(localStorage.getItem(guessed)) + 1
				: 1
		);
	} else {
...
function updateStats() {
	...
	let winPercentage = Number(
		parseInt(localStorage.getItem('wins')) /
			parseInt(localStorage.getItem('played'))
	);
...
function getCurrentRow() {
	return gridLetters.slice(
		guessIndex * WORD_LENGTH,
		guessIndex * WORD_LENGTH + WORD_LENGTH
	);
}
...
function jumpRow() {
	...
	$(toShake).each(function (i, element) {
		$(element)
			...
			.addClass('win');
		if (i == 4) fullDuration = (fullDuration + i * 0.1) * 1000;
	});
...
$('html').keydown(function (e) {
	if (e.key == 'Control') fullClear = true;
});

$('.key').on('click', function () {
	if (gameEnded) return;

	if ($(this).hasClass('delete')) removeLetter();
	else if ($(this).hasClass('enter')) submitGuess();
	else handleMouseInput($(this).text());
...
$('.letter').on('click', function (e) {
	...
	if (
		!(
			i < guessIndex * WORD_LENGTH + WORD_LENGTH &&
			i >= guessIndex * WORD_LENGTH
		)
	)
		return;
	indexSetted = true;
